...
//                    System.out.println("tcb : "+this);
//                    System.out.println("tcb here " + javaThread.getName());
                }
            });
    ...
    public void contextSwitch() {
        ...
        this.interrupt();  // 当前线程被唤醒之后 currentTCB 自然被替换了
        previous.yield();
    }
    ...
    private void threadroot() {
        ...
        if (!isFirstTCB) {
            ...
            this.yield(); // 等待 contextSwitch() 唤醒
        } else {
    ...
    private void yield() {
        ...
        currentTCB = this; // 被唤醒之后 当前TCB成为 currentTCB
    }


    // 注意java中监视器的概念，wait、notify方法调用的要保证已经获取到对象锁（synchronized）
    ...
    private synchronized void waitForInterrupt() {
        while (!running) {
            try {
                ...
                System.err.println("to here : " + e.getMessage());
            }
        }
    }
     ...
     */
    private static final Vector<TCB> runningThreads = new Vector<>();
